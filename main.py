...
from ecg_core import analyze_ecg_image, build_gemini_prompt
...
def interpret_with_gemini(
    ...
    if not GEMINI_API_KEY:
        ...
        fallback["is_valid_ecg"] = True  # DSP trace-check already passed
        return fallback

    model = genai.GenerativeModel("gemini-2.5-flash")
    prompt = build_gemini_prompt(measured, language)
    ...
    try:
        ...
        traceback.print_exc()
        fallback = _rule_based_label(measured, language)
        fallback["is_valid_ecg"] = True  # Gemini unavailable — DSP trace-check already passed
        return fallback
