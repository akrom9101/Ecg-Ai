...
def _validate_ecg_layout(img):
    ...
    most of the strip, printed in red/pink. Requiring BOTH the color cue
    AND actual counted geometric lines (not just "some reddish pixels")
    is what tells real ECG paper apart from skin tones, fabric, or other
    warm-toned photos, which can satisfy a color-only check by accident.
    """
    b, g, r = [channel.astype(np.int16) for channel in cv2.split(img)]
    redness = r - ((g + b) / 2.0)
    # Loose color gate: real ECG paper ranges from pale salmon to strong
    # pink/red, and phone photos + JPEG compression wash colors out further.
    # This only needs to reject clearly cold/neutral images (grayscale scans,
    # blue-toned photos) — the real discrimination is done below by counting
    # actual geometric grid lines, which a random warm-toned object can't fake.
    grid_mask = (r > 80) & (redness > 4) & (r >= g)
    grid_fraction = float(grid_mask.mean())
    if grid_fraction < 0.005:
        raise ValueError(
    ...
    # A real grid strip has many lines in BOTH directions. Requiring both
    # counts (not just one) rejects photos that happen to have some
    # parallel edges in only one orientation (a doorframe, a table edge).
